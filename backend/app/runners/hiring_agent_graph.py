"""LangGraph nodes for Hiring Screening agent and human-checkpoint runners."""
...
from app.runners import hiring_screening
...
RISKY_ACTIONS: set[str] = {"advance_to_interview", "reject"}
...
class AgentNodes:
    ...
    async def plan(self, state: AgentState) -> dict[str, Any]:
        ...
        response = await self._client.chat(
            state["messages"], tools=[LOOKUP_ROLE_REQUIREMENTS_TOOL_SCHEMA]
        )
    ...
    async def act(self, state: AgentState) -> dict[str, Any]:
        ...
        for call in last.get("tool_calls", []):
            ...
            role = lookup_role_requirements(role_id)
            ...
            content = json.dumps(
                role if role is not None else {"error": f"No role found with id {role_id}"}
            )
            new_messages.append(
                {"role": "tool", "tool_call_id": call["id"], "content": content}
            )

        logger.info("hiring agent graph called lookup_role_requirements: %s", tool_calls_made)
        return {
    ...
    async def decide(self, state: AgentState) -> dict[str, Any]:
        ...
        if last.get("tool_calls"):
            ...
            # lookup_role_requirements — synthesize tool responses so the
        ...
        role = _role_from_messages(state["messages"])
        ...
        for attempt in range(MAX_DECIDE_ATTEMPTS):
            ...
            try:
                ...
                proposed = hiring_screening.parse_screening_proposal(data, role)
            except (KeyError, TypeError, json.JSONDecodeError) as exc:
                logger.warning(
                    "hiring agent decide attempt %d produced bad JSON: %s", attempt + 1, exc
                )
                retries += 1
                continue
        ...
        return {
            ...
            "estimated_cost_usd": estimated_cost_usd,
            "retries": retries,
            "proposed": hiring_screening.FALLBACK_PROPOSAL,
            "node_events": [
                *state["node_events"],
                node_event(
                    "decide",
    ...
    def finalize(self, state: AgentState) -> dict[str, Any]:
        ...
        if decision is not None:
            ...
            if decision == "reject" and actions:
                ...
                actions = [hiring_screening.REJECT_ACTION]
...
def _role_from_messages(messages: list[dict[str, Any]]) -> dict[str, Any] | None:
